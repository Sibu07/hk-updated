...
from bot import bot, user, bot_cache, bot_name, config_dict, user_data, botStartTime, LOGGER, Interval, DATABASE_URL, QbInterval, INCOMPLETE_TASK_NOTIFIER, scheduler
...
async def start(client, message):
    ...
    if len(message.command) > 1 and message.command[1] == "kc":
...
async def main():
    ...
    LOGGER.info(f"KC Bot [@{bot_name}] Started!")
    if user:
        LOGGER.info(f"KC's User [@{user.me.username}] Ready!")
